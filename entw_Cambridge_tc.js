...
class entw_Cambridge_tc {
    ...
    async displayName() {
        ...
        if (locale.indexOf('TW') != -1) return '劍橋英漢雙解(繁体) v12';
    ...
    async findCambridge(word) {
        ...
        for (const entry of entries) {
            ...
            for (const sensbody of sensbodys) {
                ...
                let cssScale = scale ? `<span class='cssScale'>${scale}</span>` : '';
    ...
    renderCSS() {
        let css = `
            <style>
                ...
                span.pos  {text-transform:lowercase; font-size:0.9em; margin-right:5px; padding:2px 4px; color:white; background-color:#0d47a1; border-radius:3px;}
                span.cssScale {font-size:0.9em; margin-right:5px; padding:2px 6px; color:white; background-color:#4C3DD5; border-radius:50px;}
